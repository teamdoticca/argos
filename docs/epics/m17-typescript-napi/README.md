...
Ship **`@doticca/argos`** as a first-class Node/TypeScript package via **napi-rs**, linking `argos-core` + platform backends directly (not a JS loader over `argos_ffi`). Mirror the .NET `ArgosWorkspace` surface. Natives for **win32-x64-msvc**, **linux-x64-gnu**, and **darwin-arm64**. `WorkspaceSnapshot` remains sole business truth.

**One-line outcome:** a clean Node project can `npm install @doticca/argos`, `Workspace.open(root)`, get non-empty scopes on fixtures, and watch planned paths on Windows, Linux, and Apple Silicon.
...
- [x] Clean Node project load path documented (README + `@doticca/argos`)
