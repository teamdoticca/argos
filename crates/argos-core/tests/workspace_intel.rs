...
#[test]
fn discovers_mixed_polyglot_ops_fixture() {
    ...
    for needed in [
        ...
        assert!(
            providers.contains(&needed),
            "expected provider {needed}, got {:?}",
            providers
        );
    }
    assert!(
    ...
    assert!(
        scopes
            .iter()
            .any(|s| s.root.contains("docker-compose.yml") && s.watch.iter().any(|w| w == ".")),
        "expected compose file watch, got {:?}",
        scopes
    );
}
